...
class AirflowWebserver(Script):
	...
	def install(self, env):
		...
		Execute(format("pip install --upgrade {airflow_pip_params} pip==18.1"))
		...
		Execute(format("chmod 755 /usr/local/bin/airflow"))
		Execute(format("useradd {airflow_user}"), ignore_failures=True)
		Execute(format("mkdir -p /home/airflow && chown airflow:airflow /home/airflow"))
